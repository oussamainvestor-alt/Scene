...
export function LayoutPanel(props: LayoutPanelProps) {
  ...
  const setScreenContentScale = (value: number) => {
    const next = structuredClone(props.layout)
    next.screen.contentScale = clamp(value, 0.5, 2.0)
    props.onLayoutChange(next)
  }

  const setWorldSize = (value: number) => {
    const next = structuredClone(props.layout)
    next.worldSize = clamp(value, 1, 3)
    props.onLayoutChange(next)
  }
  ...
  const panel = (
    ...
    >
      ...
      <p className="tiny-title">Content Scale</p>
      <div className="scale-row">
        <input
          type="range"
          min="0.5"
          max="2.0"
          step="0.01"
          value={props.layout.screen.contentScale ?? 1}
        ...
        <input
          ...
          max="2.0"
          step="0.01"
          value={Number((props.layout.screen.contentScale ?? 1).toFixed(2))}
          onChange={(e) => setScreenContentScale(Number(e.target.value))}
        />
      </div>
  ...
  <p className="tiny-title">World Size</p>
  <div className="scale-row">
    <input
      type="range"
      min="1"
      max="3"
      step="0.1"
      value={(props.layout as any).worldSize ?? 1}
    ...
    <input
      ...
      max="3"
      step="0.1"
      value={Number(((props.layout as any).worldSize ?? 1).toFixed(1))}
      onChange={(e) => setWorldSize(Number(e.target.value))}
    />
  </div>
